...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(50, 0),  # All numbers are approximate.
                rg.Point(0, 50),
                rg.Point(-50, 0),
                rg.Point(0, -50)]
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points_on_circle)):
        little_circles = rg.Circle(points_on_circle[k], 10)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  10 slices, thin (thickness=2) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 160), 55)
    circle.outline_thickness = 3
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    amount_of_pts = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(amount_of_pts)):
        start = amount_of_pts[k]
        end = circle.center
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  5 segments with thick green lines;  5 with medium red lines.'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    amount_of_points = generate_points_on_circle(circle, number_of_segments)
    for k in range(number_of_segments):
        start = amount_of_points[k]
        if k == (number_of_segments - 1):
            k = -1
        end = amount_of_points[k + 1]
        line = rg.Line(start, end)
        line.color = color
...
def run_test_fancy_polygon():
  ...
  # Test 4 (on another window):
    ...
    fancy_polygon(window, circle, 15, 3, 'red', 7)
    window.close_on_mouse_click()
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    circle.attach_to(window)
    amount_of_points = generate_points_on_circle(circle, number_of_lines)
    for k in range(number_of_lines):
        start = amount_of_points[k]
        end = amount_of_points[(k + hops_to_next_point) % number_of_lines]
        line = rg.Line(start, end)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
